...
old_annotations_allowed = 500 ## annotations allowed per old class
# class_list = ['Pedestrian', 'Cyclist', 'Person_sitting', 'Tram']

# write a function that loads the dataset into detectron2's standard format
def get_kitti_dicts(root_dir, data_label, class_dict, current_class):
    ...
    class_old_dict = deepcopy(class_dict)
    del class_old_dict[current_class]
    
    old_class_track = {}
    for old_class in class_old_dict.keys():
        old_class_track[old_class] = 0
        ...
        with open(label_name) as file:
            ...
            for obj in objects:
                ...
                if obj[0] in class_labels:
                    ## if it's current class, keep it for sure
                    if obj[0] == current_class:
                        obj_ann = {
                    ...
                    ### this is the old class! 
                        ...
                        ## if we haven't exhaused budget for older classes
                        if old_class_track[obj[0]] <= old_annotations_allowed:
                            obj_ann = {
                                "bbox": [float(i) for i in obj[4:8]],
                                "bbox_mode": BoxMode.XYXY_ABS,                    
                                "category_id": class_dict[obj[0]],
                            ...
                            old_class_track[obj[0]] += 1 ## keep adding
                            objs.append(obj_ann)
...
for ind, class_name in enumerate(class_list_all):
    ...
    for d in ["train", "test"]:
        DatasetCatalog.register("kitti/" + d + "/" + class_name, lambda d=d: get_kitti_dicts(root_dir, d, class_dict_all, class_name))
