...
class FastRCNNOutputs(object):
	...
	def __init__(
		...
		if proposals[0].has("gt_weight"):
			self.gt_weight = cat([p.gt_weight for p in proposals], dim=0)
		else:
			self.gt_weight = None
	...
	def weighted_softmax_cross_entropy_loss(self):
		"""
		Compute the weighted softmax cross entropy loss for box classification.

		At this point, this is used for distillation in continual learning
		with loss attenuation outputs
		...
		cross_entropy_out = F.cross_entropy(self.pred_class_logits, self.gt_classes, reduction="none")

		## weight for background class should be 1, which is messed up, let's fix it!
		bg_class_ind = self.pred_class_logits.shape[1] - 1
		bg_inds = (self.gt_classes == bg_class_ind)

		gt_weights = self.gt_weight.clone()
		gt_weights[bg_inds] = 1.0

		## let's contribute according to the weights
		loss_class = cross_entropy_out * gt_weights

		## sum and normalize
		loss_class = loss_class.sum() / gt_weights.sum()
	...
	def loss_attenuation(self):
		...
		if cls_agnostic_bbox_reg:
			...
			if self.gt_weight is not None:
				gt_weights = self.gt_weight[fg_inds]
			# pred_proposal_deltas for class k are located in columns [b * k : b * k + b],
			# where b is the dimension of box representation (4 or 5)
			# Note that compared to Detectron1,
			# we do not perform bounding box regression for background classes.
		...
		## if ground truth no weight
		if self.gt_weight is None:
		...
		## if ground truth has weight

		else:

			'''
				Remember that we should multiply only regression term with the weight
				and not the entire loss term. Because loss attenuation can be negative,
				in that case multiplying it with a weight(which is < 1) will result in 
				more loss, effectively making the noisy labels to have more impact on 
				the training procedure. The noisy labels should have weight less than
				1 and they shouldn't contribute much to the overall loss much as it may
				end up teaching wrong thing to the network.
			'''
			loss_attenuation_final = ((((self.pred_proposal_deltas[fg_inds[:, None], gt_class_cols] - gt_proposal_deltas[fg_inds])**2) * gt_weights[:, None])/(self.pred_proposal_uncertain[fg_inds[:, None], gt_class_cols]) + torch.log(self.pred_proposal_uncertain[fg_inds[:, None], gt_class_cols]))
			loss_attenuation_final = loss_attenuation_final.sum() / self.gt_classes.numel()
	...
	def losses(self):
		...
		if self.gt_weight is None:
			...
			loss_cls = self.weighted_softmax_cross_entropy_loss()
